...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion < 2) createLedgerTable(db);
        if (oldVersion < 3) createChatTable(db);
    ...
    public synchronized boolean sellAsset(String symbol, double quantity) {
        ...
        if (!c.moveToFirst()) { c.close(); db.close(); return false; }

        double existQty = c.getDouble(c.getColumnIndexOrThrow(C_QUANTITY));
        c.close();

        if (existQty < quantity - 1e-9) { db.close(); return false; }
    ...
    public double getQuantity(String symbol) {
        ...
        double qty = 0;
        if (c.moveToFirst()) qty = c.getDouble(0);
        c.close();
        db.close();
        return qty;
    }
    ...
    public List<Transaction> getRecentLedger(int limit) {
        ...
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.query(LEDGER, null, null, null, null, null,
        ...
        while (c.moveToNext()) {
            ...
            String amtStr  = credit
                    ? String.format(java.util.Locale.getDefault(), "+$%,.2f", amount)
                    : String.format(java.util.Locale.getDefault(), "-$%,.2f", amount);

            list.add(new Transaction(icon != null ? icon : "💱", title, dateStr, amtStr, credit));
        }
        c.close();
        db.close();
        return list;
    ...
    public float[][] getMonthlyCashFlow(int months) {
        ...
        for (int i = 0; i < months; i++) {
            ...
            while (c.moveToNext()) {
                ...
                if (credit) result[i][0] = (float) amt;
                else         result[i][1] = (float) amt;
            }
            c.close();
        }
        db.close();
    ...
    public List<FrequentContact> getContacts() {
        ...
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.query(CONTACTS, null, null, null, null, null,
                CO_USED + " DESC", "8");
        while (c.moveToNext()) {
